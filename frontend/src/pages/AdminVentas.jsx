    ...
    return (
      <div style={st.page}>
        <div style={st.loginBox} data-testid="admin-login">
          ...
          {error && <div style={st.error} data-testid="admin-login-error">{error}</div>}
      ...
      <div style={st.wrap}>
        <div style={st.top}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button style={st.btnGhost} onClick={() => cargar(key)} disabled={loading} data-testid="admin-refresh-btn">
              {loading ? "Actualizando..." : "↻ Actualizar"}
            </button>
            <button style={st.btnGhost} onClick={salir} data-testid="admin-logout-btn">Salir</button>
          </div>
        </div>
        ...
        <p style={{ ...st.muted, marginTop: 16 }}>
          Solo se muestran transacciones con estado APPROVED reportadas por Wompi.
        </p>
      </div>
    </div>
  );
